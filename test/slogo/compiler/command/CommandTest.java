...
class CommandTest {
  ...
  void testMovementCommands() {
    ...
    assertThrows(SLogoException.class, () -> function = new SLogoFunction(parameterTokens, modelTurtle));
    ...
    assertThrows(SLogoException.class, () -> function = new SLogoFunction(parameterTokens,
        modelTurtle));
  ...
  void testBinaryTurtleCommandsAndQueries() {
    ...
    function = new SLogoFunction(parameterTokens, modelTurtle); // run again to show it does nothing
    assertEquals(1.0, function.run().getValue());
    assertEquals(1.0, modelTurtle.showingP());
  ...
  void testTwoArgumentMathCommands() {
    ...
    function = new SLogoFunction(parameterTokens, modelTurtle); // should work, not a compiler error
  ...
  void testOneArgumentMathCommands() {
    ...
    command.resetCommand();
    parameterTokens.add(new SLogoConstant(-100));
    function = new SLogoFunction(parameterTokens, modelTurtle); // not a compiler error
    assertThrows(SLogoException.class, () -> function.run());
  }
  ...
  void testTwoArgumentBooleanCommands() {
    ...
    SLogoFunction piFunction = new SLogoFunction(new ArrayDeque<>(), modelTurtle);
